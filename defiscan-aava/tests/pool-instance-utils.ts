...
export function createDeficitCoveredEvent(
  reserve: Address,
  caller: Address,
  amountCovered: BigInt
): DeficitCovered {
  let deficitCoveredEvent = changetype<DeficitCovered>(newMockEvent())

  deficitCoveredEvent.parameters = new Array()

  deficitCoveredEvent.parameters.push(
    new ethereum.EventParam("reserve", ethereum.Value.fromAddress(reserve))
  )
  deficitCoveredEvent.parameters.push(
  ...
  deficitCoveredEvent.parameters.push(
    new ethereum.EventParam(
      "amountCovered",
      ethereum.Value.fromUnsignedBigInt(amountCovered)
    )
  )
...
export function createDeficitCreatedEvent(
  user: Address,
  debtAsset: Address,
  amountCreated: BigInt
): DeficitCreated {
  let deficitCreatedEvent = changetype<DeficitCreated>(newMockEvent())

  deficitCreatedEvent.parameters = new Array()

  deficitCreatedEvent.parameters.push(
    new ethereum.EventParam("user", ethereum.Value.fromAddress(user))
  )
  deficitCreatedEvent.parameters.push(
  ...
  deficitCreatedEvent.parameters.push(
    new ethereum.EventParam(
      "amountCreated",
      ethereum.Value.fromUnsignedBigInt(amountCreated)
    )
  )
